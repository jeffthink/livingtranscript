	...
	$.fn.extend({
		tagcloud: function(_settings) {
			...
			switch (oSettings.type) {
				...
				case "list":
					oUlCss = {height:"auto"};
					oLiCss = {position:"static",display:"inline-block"};
				break;
			}
...
//				var iUlH = oSettings.height===null?mUl.height():oSettings.height;
				...
				mUl.css({width:iUlW,height:iUlH,listStyle:"none",margin:"0 10px 0 0",padding:0});
				mUl.css(oUlCss);
				...
				for (var j=0;j<iNumLi;j++) {
					...
					//
					mLi.css({"fontSize":iSzFnt,position:"absolute",color:"#"+sColor,margin:"0 10px 0 0",padding:0}).children().css({color:"#"+sColor});
					var iLiW = mLi.width();
					var iLiH = mLi.height()
					//
					var oCss = {};
